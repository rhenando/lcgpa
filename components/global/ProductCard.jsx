...
const ProductCard = ({ product, currencySymbol, formatNumber }) => {
  ...
  // Localize value by object or string
  ...
  return (
    <div dir={isRTL ? "rtl" : "ltr"} className={`p-1 sm:p-2 relative`}>
      {isPending && (
      ...
      <div
        className={`relative group bg-white border rounded-xl shadow hover:shadow-md transition-all flex flex-col overflow-hidden
          ${isRTL ? "text-right" : "text-left"}`}
      >
        {/* Heart (Favorite) icon: right/left */}
        <div
          className={
            isRTL ? "absolute top-2 left-2 z-10" : "absolute top-2 right-2 z-10"
          }
        >
        ...
        <div
          className={
            isRTL
              ? "absolute top-2 right-2 bg-red-600 text-white text-[0.5rem] sm:text-xs font-semibold px-2 py-0.5 rounded shadow z-10"
              : "absolute top-2 left-2 bg-red-600 text-white text-[0.5rem] sm:text-xs font-semibold px-2 py-0.5 rounded shadow z-10"
          }
        >
          {t("hot")}
        </div>
        <div
          className='relative aspect-[4/3] bg-white overflow-hidden border-2 border-[#2c6449] rounded-xl cursor-pointer'
        ...
        <div className='flex flex-col p-2 sm:p-4 flex-1 bg-white'>
          <div className='flex-1'>
            ...
            <p className='text-[0.6rem] sm:text-xs text-gray-500 mt-1 mb-2'>
              ...
              <span className='capitalize font-medium'>
                {product.supplierName ||
                  (isRTL ? t("unknown", { ns: "product_card" }) : t("unknown"))}
              </span>
            </p>

            {/* Price ranges (localized) */}
            {priceRanges.length > 0 ? (
              <div className='mb-1'>
                {priceRanges.map((tier, idx) => {
                  ...
                  const isUnlimited =
                    (typeof max === "string" &&
                      (max.trim().toLowerCase() === "unlimited" ||
                        (isRTL && max.trim() === "غير محدود"))) ||
                    max === Infinity;

                  const rangeLabel = isUnlimited
                    ? `${formatLocalizedNumber(min)}–${
                        isRTL ? t("unlimited") : t("unlimited")
                      } ${isRTL ? t("pcs") : "Pcs"}`
                    : Number(max) >= Number(min)
                    ? `${formatLocalizedNumber(min)}–${formatLocalizedNumber(
                        max
                      )} ${isRTL ? t("pcs") : "Pcs"}`
                    : `${formatLocalizedNumber(min)}+ ${
                        isRTL ? t("pcs") : "Pcs"
                  ...
                  return (
                    ...
                    >
                      ...
                      {isNaN(priceVal) ? (
                        <span className='text-sm italic text-[#2c6449]'>
                          {t("negotiable")}
                        </span>
                      ) : (
            ...
            <p className='text-[0.6rem] sm:text-xs text-gray-500 capitalize'>
              {t("min_order", { minOrder: formatLocalizedNumber(minOrder) })}
            </p>
          </div>

          {/* Buttons order reversed in RTL */}
          <div
            className={`flex flex-col sm:flex-row gap-1 sm:gap-2 mt-4 ${
