  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      snap.forEach((docSnap) => {
        ...
        const catEn = typeof category === "object" ? category?.en : category;
        const subEn =
          typeof subCategory === "object" ? subCategory?.en : subCategory;

        if (!catEn) return;
        if (!map[catEn]) map[catEn] = new Set();
        if (subEn) map[catEn].add(subEn);
      });
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        const categoryValue =
          typeof data.category === "object" ? data.category.en : data.category;
        ...
        setSelectedSubCategory(
          subCategoryValue
            ? { label: subCategoryValue, value: subCategoryValue }
            : null
        );
  ...
  if (loading) {
    return (
      <div className='p-6 text-center'>
        <Loader2 className='animate-spin mx-auto' /> Loading product…
      </div>
    );
  }
