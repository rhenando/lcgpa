...
// Helper to get first non-empty value from possible field names
function getSupplierField(supplier, ...fields) {
  if (!supplier) return "—";
...
export default function SupplierDetailsPage() {
  ...
  // Renders either an input (in edit mode) or text/link. Uses getSupplierField for fallbacks.
  const renderField = (
    label,
    field,
    type = "text",
    ...
    <div className='flex flex-col'>
      ...
      {isEditing ? (
        ...
        getSupplierField(supplier, field, ...fallbacks) &&
        getSupplierField(supplier, field, ...fallbacks) !== "—" ? (
        <a
          href={getSupplierField(supplier, field, ...fallbacks)}
          target='_blank'
          rel='noreferrer'
        ...
        <span>{getSupplierField(supplier, field, ...fallbacks)}</span>
      )}
    </div>
  );
      ...
      <Card className='mb-6'>
        <CardHeader>
          <CardTitle className='text-xl'>
            {getSupplierField(supplier, "companyName", "companyNameAr", "name")}
          </CardTitle>
        </CardHeader>
        <CardContent className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          ...
          <div className='flex flex-col'>
            ...
            <span>{getSupplierField(supplier, "role", "designation")}</span>
          </div>
          <div className='flex flex-col'>
            <label className='font-medium mb-1'>UID</label>
            <span>{getSupplierField(supplier, "uid", "id")}</span>
          </div>
          ...
          {renderField("Phone", "phone", "tel", null, [
            "companyPhone",
            "authPersonMobile",
            "representativePhone",
          ])}
          ...
          {renderField("Company Name", "companyName", "text", null, [
            "companyNameAr",
          ])}
          {renderField("Description", "description", "text", null, [
            "companyDescriptionEn",
            "companyDescriptionAr",
          ])}
          {renderField("Full Address", "fullAddress", "text", null, [
            "address",
            "addressAr",
          ])}
          ...
          {renderField("CR Number", "crNumber", "text", null, [
            "commercialReg",
          ])}
          {renderField("PID Number", "pidNumber", "text", null, [
            "personalIdNumber",
          ])}
          {renderField("Unified Number", "unifiedNumber", "text", null, [
            "nun",
          ])}
          ...
          {renderField("VAT Number", "vatNumber", "text", null, [
            "vatRegNumber",
          ])}
          ...
          <div className='flex flex-col'>
            ...
            {isEditing ? (
              ...
              <span>
                {getSupplierField(supplier, "isApproved", "approved")
                  ? "✅ Yes"
                  : "❌ No"}
              </span>
            )}
          ...
          <div className='flex flex-col'>
            ...
            {isEditing ? (
              <textarea
                value={
                  Array.isArray(form.productImages)
                    ? form.productImages.join("\n")
                    : ""
                }
                onChange={(e) =>
                  setForm((f) => ({
