...
export default function SupplierUsersPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        toast.error(
          "Failed to fetch suppliers. Check security rules and admin status."
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = (uid) => {
    toast.promise(
      (async () => {
        // Use the new relative API path
        const res = await fetch(`/api/delete-supplier/${uid}`, {
          method: "DELETE",
        });
        ...
        // Update UI after server has wiped everything
    ...
    toast.promise(
      (async () => {
        ...
        const res = await fetch(
          `/api/admin/approve-and-authenticate-supplier/${docId}`,
          { method: "PUT" }
        );
